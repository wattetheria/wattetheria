...
function npmGlobalRoot() {
  const result = spawnSync(npmCommandName(), ["root", "-g"], {
    encoding: "utf8"
  });
  if (result.error) {
    return "";
...
function isGlobalNpmInstall() {
  ...
  return Boolean(root) && pathIsInsideOrEqual(PACKAGE_ROOT, root);
}
...
function updateCliPackage() {
  ...
  if (!isGlobalNpmInstall()) {
    console.log("");
    console.log("This CLI is not running from a global npm install.");
    console.log("Update it manually with:");
    console.log(`  ${command}`);
    return;
  }
