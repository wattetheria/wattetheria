...
const RELEASE_CONTAINER_STATE_UID = "10001";
const RELEASE_DEPLOY_DIRECTORY_MODE = 0o777;
const RELEASE_DEPLOY_FILE_MODE = 0o666;
const HTTP_CHECK_REQUEST_TIMEOUT_MS = 5000;
...
Options:
  ...
  --data-dir <path>      With mcp-proxy, doctor, or network, override node state
...
Network:
  ...
  Commands:
    authority-init       Initialize the dedicated Network Authority DID
    authority-show       Show the Network Authority public identity
    create-request       Create a signed autonomous-network registration request
    inspect-request      Verify and inspect a registration request
    export-trust-bundle  Export the mainnet trust anchor
    issue-credential     Issue a Network Membership Credential
    verify-credential    Verify a membership credential and its evidence
    import-credential    Verify and store a membership credential
    list-credentials     List locally stored membership credentials
    revoke-credential    Sign and store a credential revocation
    verify-revocation    Verify a signed credential revocation
    import-revocation    Verify and apply a credential revocation
`);
}
...
function runNpm(args, options = {}) {
  return spawnSync(npmCommandName(), args, {
    ...options,
...
function latestPublishedCliVersion() {
  const result = runNpm(["view", "wattetheria", "version"], {
    encoding: "utf8"
  });
  if (result.error) {
...
function ensureCliVersionIsLatest(commandName = "install") {
...
function ensureDeploymentAssets(options) {
  ...
  ensureReleaseDeployDirectoryAccess(options.dir);
}
...
function ensureReleaseDeployDirectoryAccess(deployDir) {
  if (process.platform !== "linux") {
    return;
  }
  if (grantReleaseDeployDirectoryAcl(deployDir)) {
    return;
  }
  try {
    chmodReleaseDeployDirectoryTree(deployDir);
  } catch (error) {
    throw new Error(
      [
        `Unable to prepare release deploy directory for container writes: ${deployDir}`,
        `Grant UID ${RELEASE_CONTAINER_STATE_UID} read/write access to the deploy directory, then rerun setup.`,
...
function grantReleaseDeployDirectoryAcl(deployDir) {
  if (process.platform !== "linux") {
    return false;
  }
  const result = spawnSync(
    ...
    [
      "-c",
      [
        `setfacl -R -m "u:${RELEASE_CONTAINER_STATE_UID}:rwx" "$1"`,
        "&&",
        `find "$1" -type d -exec setfacl -m "d:u:${RELEASE_CONTAINER_STATE_UID}:rwx" {} +`
      ].join(" "),
      "sh",
      deployDir
    ],
...
function chmodReleaseDeployDirectoryTree(deployDir) {
  chmodReleaseDeployPath(deployDir);
  for (const entry of fs.readdirSync(deployDir, { withFileTypes: true })) {
    chmodReleaseDeployEntry(path.join(deployDir, entry.name), entry);
  }
}

function chmodReleaseDeployEntry(entryPath, entry) {
  if (entry.isSymbolicLink()) {
    return;
  }
  chmodReleaseDeployPath(entryPath);
  if (!entry.isDirectory()) {
    return;
  }
  for (const child of fs.readdirSync(entryPath, { withFileTypes: true })) {
    chmodReleaseDeployEntry(path.join(entryPath, child.name), child);
  }
}

function chmodReleaseDeployPath(targetPath) {
  const stat = fs.lstatSync(targetPath);
  const mode = stat.isDirectory()
    ? stat.mode | RELEASE_DEPLOY_DIRECTORY_MODE
    : stat.mode | RELEASE_DEPLOY_FILE_MODE;
...
async function waitForHttp(name, url, maxAttempts = 60, delayMs = 2000) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      const response = await fetchWithTimeout(url, HTTP_CHECK_REQUEST_TIMEOUT_MS);
      if (response.ok) {
      ...
      lastError = error.name === "AbortError"
        ? `request timed out after ${HTTP_CHECK_REQUEST_TIMEOUT_MS}ms`
        : error.message;
    }
    if (attempt === 1 || attempt % 5 === 0) {
      console.log(`Still waiting for ${name} (${attempt}/${maxAttempts}): ${lastError}`);
    }
    await new Promise((resolve) => setTimeout(resolve, delayMs));
  }
  throw new Error(
    `Timed out waiting for ${name}: ${url}${lastError ? ` (${lastError})` : ""}`
  );
}
...
function printAgentRuntimeApiServerInstructions() {
  console.log("Start an agent runtime API server before saving Runtime config.");
  console.log("");
  console.log("Wattetheria expects an OpenAI-compatible runtime API:");
  console.log("  GET  <base_url>/models");
  console.log("  POST <base_url>/chat/completions");
  console.log("");
  console.log("Use Bearer token auth when your runtime requires an API key.");
  console.log("Use a stable-session header when your selected adapter requires one.");
  console.log("");
  console.log("Examples:");
  console.log("  Hermes:   http://host.docker.internal:8642/v1, model hermes-agent");
  console.log("  OpenClaw: http://host.docker.internal:18789/v1, model openclaw/default");
  console.log("  Custom:   your runtime's OpenAI-compatible /v1 base URL and model id");
}

function printManualSetupChecklist(options) {
  console.log("");
  console.log("Finish setup:");
  console.log("1. Start an agent runtime API server:");
  printAgentRuntimeApiServerInstructions();
  console.log("");
  console.log("2. Open Supervision and configure the runtime:");
  console.log(`   ${supervisionUrl(options)}`);
  console.log("");
  console.log("3. Add Wattetheria MCP to your agent runtime:");
  console.log(mcpServerConfigSnippet(options));
  console.log("");
  console.log("4. Restart Wattetheria so runtime config takes effect:");
  console.log("   npx wattetheria restart");
  console.log("");
  console.log("5. Restart your agent runtime so MCP config takes effect.");
  console.log("");
  console.log("6. Verify MCP access from the agent runtime:");
...
async function setup(options) {
  ensureCliVersionIsLatest("setup");
  ...
  if (deployment.runnable) {
    console.log("[1/7] Start existing Wattetheria deployment");
    ...
    console.log("[1/7] Install Wattetheria");
    await install(options, {
      cliVersionAlreadyChecked: true,
  ...
  console.log("");
  console.log("[2/7] Start an agent runtime API server");
  printAgentRuntimeApiServerInstructions();
  await waitForEnter("After starting the API server and noting its base URL, model, API key, and session header, press Enter to continue.");

  console.log("");
  console.log("[3/7] Configure runtime");
...
async function update(options) {
  ensureCliVersionIsLatest("update");
  await ensureDockerAvailable();
  if (!fs.existsSync(composeFilePath(options)) || !fs.existsSync(envFilePath(options))) {
    throw new Error("Deployment is not initialized. Run install first.");
  }
...
function updateCliPackage() {
  ...
  const result = runNpm(["install", "-g", "wattetheria@latest"], {
    stdio: "inherit"
  });
...
async function mcpProxy(options) {
  ...
  for await (const line of input) {
    ...
    const response = await forwardMcpRequest(endpoint, token, request, negotiatedProtocolVersion);
    if (
...
async function forwardMcpRequest(endpoint, token, request, negotiatedProtocolVersion = null) {
  const headers = {
    "accept": "application/json, text/event-stream",
    "content-type": "application/json"
  };
  if (request.method !== "initialize" && negotiatedProtocolVersion) {
...
async function run(argv) {
  ...
  if (argv[0] === "network") {
    forwardToRustBinary("network", argv);
    return;
  }
