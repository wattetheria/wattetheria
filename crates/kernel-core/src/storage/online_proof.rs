...
impl OnlineProofManager {
    ...
    pub fn create_lease(
        ...
        self.leases.insert(agent_id.to_string(), lease.clone());
        lease
    }

    pub fn heartbeat(&mut self, agent_id: &str) -> Option<Lease> {
        let now = Utc::now().timestamp();
        let lease = self.leases.get_mut(agent_id)?;
        lease.last_heartbeat = now;
        lease.missed_heartbeats = 0;
        Some(lease.clone())
    }

    pub fn spot_check(&mut self, agent_id: &str, skew_tolerance_sec: i64) -> SpotCheckStatus {
        let Some(lease) = self.leases.get_mut(agent_id) else {
    ...
    #[must_use]
    pub fn get_proof(&self, agent_id: &str) -> Option<serde_json::Value> {
        let lease = self.leases.get(agent_id)?;
