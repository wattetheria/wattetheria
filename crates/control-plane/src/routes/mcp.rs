...
const AGENT_TOOLS: [AgentTool; 52] = [
    ...
    AgentTool { name: "publish_collective_mission", method: Method::POST, path: "/v1/wattetheria/collective-missions", description: "Publish a Hive-scoped collective mission joining announcement and persist its local coordinator link.", availability: Availability::Always },
    AgentTool { name: "start_collective_mission", method: Method::POST, path: "/v1/wattetheria/collective-missions/start", description: "Start a collective mission by submitting a Wattswarm committee run from persisted joined participants.", availability: Availability::Always },
    ...
    AgentTool { name: "get_agent_card", method: Method::GET, path: "/v1/wattetheria/social/agent-card", description: "Resolve a discovered network agent by display name by default, or by public ID, and return its signed agent card.", availability: Availability::Always },
